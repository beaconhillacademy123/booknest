...
export default function AdminPage(){
  ...
  async function load(){
    ...
    setAllowed(true);
    const {data}=await supabase.from('booknest_book_requests').select('id,title,author,source_url,reason,status,created_at').order('created_at',{ascending:false});
  ...
  {rows.length===0?<div className="empty">No book suggestions yet.</div>:<div className="adminRequests">{rows.map(r=><article className="adminRequest" key={r.id}><div><div className="adminStatus"><Clock3 size={14}/>{r.status}</div><h3>{r.title}</h3><p className="author">{r.author||'Author not provided'}</p>{r.reason&&<p>{r.reason}</p>}{r.source_url&&<a href={r.source_url} target="_blank" rel="noreferrer">Open source</a>}</div><div className="adminActions">{r.status==='pending'&&<><button onClick={()=>setStatus(r.id,'approved')}><Check size={16}/> Approve</button><button onClick={()=>setStatus(r.id,'rejected')}><X size={16}/> Reject</button></>}</div></article>)}</div>}</section></main>
}