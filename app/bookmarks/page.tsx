...
export default function BookmarksPage(){
 const [rows,setRows]=useState<Row[]>([]); const [loading,setLoading]=useState(true); const [removing,setRemoving]=useState<string|null>(null);
 ...
 async function remove(id:string){if(removing)return;setRemoving(id);const {error}=await supabase.from('booknest_bookmarks').delete().eq('id',id);if(!error)setRows(r=>r.filter(x=>x.id!==id));setRemoving(null);}
